...
export default function AttendanceDetailPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            const { data: enrollments, error: enrollError } = await supabase
                ...
                .eq('status', 'active')
                .order('student_id') // Consistent ordering
    ...
    const handleStatusChange = async (studentId: string, status: string) => {
        ...
        try {
            ...
            // Get instructor ID if user is instructor (optional, for marked_by)
            // For now, we can just use the user ID if the schema allows, or fetch instructor ID.
            // Assuming 'marked_by_instructor_id' expects an instructor UUID.
            ...
            const { error } = await supabase
                ...
                .upsert({
                    ...
                    status: status,
                    marked_by_instructor_id: instructorId,
                    marked_at: new Date().toISOString()
                }, { onConflict: 'class_day_id, student_id' })

            if (error) throw error

            // Success toast (debounced or subtle)
            // toast.success("Saved") 
        } catch (error) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col gap-6">
                <Link href="/admin/attendance" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-900 transition-colors">
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-2 text-sm text-gray-500">
                            ...
                            Instructor: <span className="font-medium text-gray-900">{classDay.classes?.instructors?.full_name}</span>
                        </div>
                    </div>
